#!/usr/bin/env python3
"""
Page-wise XGBoost Training Script with Oversampling

Trains XGBoost classifier on pre-extracted features for page-wise readability classification
with dataset balancing using oversampling techniques (SMOTE, ADASYN, Random Oversampling).
"""
...
class PagewiseXGBoostTrainerOversampled:
    def __init__(self, embeddings_dir, experiment_dir, backbone_name='efficientnet', sampling_strategy='smote'):
        ...
        # Log configuration
        self.logger.info(f"🚀 Initializing PAGE-WISE XGBoost Trainer with Oversampling")
    ...
    def get_oversampler(self):
        """Get the appropriate oversampling technique."""
        ...
        if self.sampling_strategy not in oversamplers:
            self.logger.warning(f"⚠️ Unknown sampling strategy: {self.sampling_strategy}. Using SMOTE.")
            return oversamplers['smote']
            
        return oversamplers[self.sampling_strategy]
    ...
    def load_pagewise_features(self):
        ...
        # Log class distribution before oversampling
    ...
    def apply_oversampling(self, X_train, y_train):
        ...
        self.logger.info(f"⚖️ Applying oversampling technique: {self.sampling_strategy}")
        
        oversampler = self.get_oversampler()
        if oversampler is None:
            self.logger.error("❌ Could not initialize oversampler")
        ...
        try:
            ...
            self.logger.error(f"❌ Oversampling failed: {str(e)}")
    ...
    def train_xgboost(self, X_train, y_train, X_test, y_test):
        ...
        X_train_resampled, y_train_resampled = self.apply_oversampling(X_train, y_train)
    ...
    def save_results(self, results, training_time):
        ...
        with open(metrics_file, 'w') as f:
            f.write("PAGE-WISE READABILITY CLASSIFICATION - XGBoost with Oversampling\n")
            f.write("=" * 70 + "\n\n")
        ...
        detailed_results = {
            ...
            'model_type': f'xgboost_{self.backbone_name}_oversampled_{self.sampling_strategy}',
    ...
    def run_training(self):
        ...
        try:
            ...
            # Train model (with oversampling applied internally)
...
def main():
    parser = argparse.ArgumentParser(description='Train XGBoost on page-wise features with oversampling')
    ...
    parser.add_argument('--sampling', type=str, default='smote',
                      choices=['smote', 'adasyn', 'random', 'smote_enn', 'smote_tomek'],
                      help='Oversampling strategy to use')
    ...
    experiment_dir = Path(f'readability_training_pagewise/experiments/xgboost_{backbone}_{sampling_strategy}_nonreadable_{timestamp}')
    
    print("🚀 Starting PAGE-WISE XGBoost training with oversampling...")
    ...
    try:
        # Initialize trainer
        trainer = PagewiseXGBoostTrainerOversampled(
        ...
        print("✅ PAGE-WISE XGBoost training with oversampling completed!")
